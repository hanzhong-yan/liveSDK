...
+ (void)presentFromViewController:(UIViewController *)viewController withTitle:(NSString *)title URL:(NSURL *)url  parameters:(NSDictionary *)parameters completion:(void (^)())completion {
    ...
    [viewController presentViewController:[[IJKVideoViewController alloc] initWithURL:url parameters:parameters] animated:YES completion:completion];
}
...
- (instancetype)initWithURL:(NSURL *)url parameters:(NSDictionary *)parameters{
    ...
    if (self) {
        self.url = url;
        transitView = [parameters objectForKey:@"transmitImgView"];
    }
    return self;
}
...
//    [[UIApplication sharedApplication] setStatusBarOrientation:UIInterfaceOrientationLandscapeLeft animated:NO];
    ...
    //add 过渡的图
//    UIImage *transitImage = [UIImage imageNamed:@"pic2.jpg" inBundle:nil compatibleWithTraitCollection:nil];
//    transitView = [[UIImageView alloc] initWithImage:transitImage];
    //transitView.center = self.view.center;
    transitView.backgroundColor = [UIColor blueColor];
    [self.view addSubview:transitView];
    
    [transitView setTranslatesAutoresizingMaskIntoConstraints:NO];
    
    //子view的上边缘离父view的上边缘40个像素
    NSLayoutConstraint *contraint1 = [NSLayoutConstraint constraintWithItem:transitView attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:self.view attribute:NSLayoutAttributeTop multiplier:1.0 constant:0.0];
    //子view的左边缘离父view的左边缘40个像素
    NSLayoutConstraint *contraint2 = [NSLayoutConstraint constraintWithItem:transitView attribute:NSLayoutAttributeLeft relatedBy:NSLayoutRelationEqual toItem:self.view attribute:NSLayoutAttributeLeft multiplier:1.0 constant:0.0];
    //子view的下边缘离父view的下边缘40个像素
    NSLayoutConstraint *contraint3 = [NSLayoutConstraint constraintWithItem:transitView attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationEqual toItem:self.view attribute:NSLayoutAttributeBottom multiplier:1.0 constant:-0.0];
    //子view的右边缘离父view的右边缘40个像素
    NSLayoutConstraint *contraint4 = [NSLayoutConstraint constraintWithItem:transitView attribute:NSLayoutAttributeRight relatedBy:NSLayoutRelationEqual toItem:self.view attribute:NSLayoutAttributeRight multiplier:1.0 constant:-0.0];
    //把约束添加到父视图上
...
-(void)installMovieNotificationObservers
{
	...
	[[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(moviePlayBackStateDidChange:)
                                               ...
                                               object:_player];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(didAfterVedioOK:)
                                                 name:IJKMPMoviePlayerFirstVideoFrameRenderedNotification
                                               object:_player];
}

-(void)didAfterVedioOK :(NSNotification*)notification{
    transitView.hidden = true;
}
